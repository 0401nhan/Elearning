...
async function seedTest(connection, employees) {
  await connection.query(
    `
    INSERT INTO tests
      (code, title, description, question_count, duration_minutes, pass_score, max_official_attempts,
    ...
    VALUES (?, 'Deep smoke attempt flow', 'Temporary automated test for concurrent attempt flows.', ?, 10, 80, 1, 1, 0, 0, 1, 0, 'active', NULL)
    `,
    [testCode, questionCount]
  );
...
async function submitOfficial(cookie, username, attempt) {
  ...
  assertCondition(body.score === 100, `Expected score 100 for ${username}, got ${body.score}.`);
...
async function runParticipant(employee, testId) {
  ...
  const result = await submitOfficial(cookie, employee.username, attempt);
...
async function assertDatabaseState(connection, testId) {
  ...
  assertCondition(Number(attemptCounts.perfect) === participantCount, "DB perfect score count mismatch.");
...
async function main() {
  ...
  try {
    ...
    const results = await Promise.all(employees.map((employee) => runParticipant(employee, testId)));
